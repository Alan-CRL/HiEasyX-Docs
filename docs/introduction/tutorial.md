...
请确保您已经配置好了 HiEasyX 库。如果没有，请查看[配置 HiEasyX](configure\configure.md)
...
???+ warning "注意"
    请不要偷懒在程序中加入 `using namespace hiex`，因为这会导致重名错误。
    正确用法：在 HiEasyX 函数前面加入 `hiex::` 即可。
...
??? bug "正在调查的漏洞"
    ...
    如果您出现了此问题，请将 `exit(0);` 替换成 `abort();`（强制退出程序）即可。
